...
const FivechanQuote = ({ quotedCids, reference }: FivechanQuoteProps) => {
  ...
  const label = authorLabel ? t('quoting_user', { author: authorLabel }) : t('fivechan_quote', { reference: reference.raw, interpolation: { escapeValue: false } });

  return (
    <span>
      {quotedCommentPath ? <Link to={quotedCommentPath}>[{label}]</Link> : <span>[{label}]</span>}
      {/* The quote only needs explaining when Seedit could not identify the comment it points at; a resolved quote links to it. */}
      {!quotedComment && <InfoTooltip content={t('fivechan_quote_tooltip', { reference: reference.raw, interpolation: { escapeValue: false } })} />}
    </span>
  );
};
